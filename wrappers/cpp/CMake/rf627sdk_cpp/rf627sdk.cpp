 ...
 * @return If successful - the number of bytes received. On failure, it returns a value of -1
 */
rfSsize platform_recv(void* socket, void *buf, rfSize len)
{
    rfSsize nret;
    if (*(SOCKET*)socket == INVALID_SOCKET) {
        return -1;
...
int SDK::RF_SCANNERS::sdk_version()
{
    /*
...
rf627_list rf627old::search(PROTOCOLS protocol)
{
    switch (protocol) {
...
profile_t* rf627old::get_profile(PROTOCOLS protocol)
{
    ...
    case PROTOCOLS::SERVICE_PROTOKOL:
    {
        ...
        profile_t* result =
                (profile_t*)memory_methods.rf_calloc(1, sizeof(profile_t));
